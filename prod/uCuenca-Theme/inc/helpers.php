 ...
 * @return void
 */
function ucuenca_theme_post_meta() {
	printf(
		/* translators: %s es la fecha de la publicación */
		esc_html__( 'Posteado en %s', 'ucuenca_theme' ),
	...
	printf(
		...
		esc_html__( ' por %s', 'ucuenca_theme' ),
 ...
 * @requires void
 */
function ucuenca_theme_read_more() {
	?>
	<div class="post-meta">
		<a href="<?php echo esc_url( get_the_permalink() ); ?> " title="<?php the_title_attribute(); ?>">
			<?php esc_html_e( 'Leer más ', 'ucuenca_theme' ); ?> <span class="u-screen-reader-text"><?php esc_html_e( 'acerca de ', 'ucuenca_theme' ) . the_title(); ?> </span>
		</a>
	</div>
	<?php
}
 ...
 * @requires void
 */
function ucuenca_theme_menu_facultad( $id_facultad = null ) {
